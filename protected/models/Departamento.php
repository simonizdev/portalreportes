...
/**
 * This is the model class for table "TH_DEPARTAMENTO".
 *
 * The followings are the available columns in table 'TH_DEPARTAMENTO':
...
class Departamento extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('Id_Departamento, Id_Usuario_Creacion, Id_Usuario_Actualizacion, Codigo, Descripcion, Estado, Fecha_Creacion, Fecha_Actualizacion, usuario_creacion, usuario_actualizacion, orderby', 'safe', 'on'=>'search'),
		);
	}

	public function searchByDepartamento($filtro) {
        
        $resp = Yii::app()->db->createCommand("
		    SELECT TOP 10 Id_Departamento, CONCAT (Codigo, ' - ', Descripcion) AS DESCR FROM TH_DEPARTAMENTO WHERE (Codigo LIKE '%".$filtro."%' OR Descripcion  LIKE '%".$filtro."%') AND Estado = 1 ORDER BY DESCR 
		")->queryAll();
        return $resp;
 	...
 	public function searchById($filtro) {
        ...
        $resp = Yii::app()->db->createCommand("
		    SELECT Id_Departamento , CONCAT (Codigo, ' - ', Descripcion) AS DESCR FROM TH_DEPARTAMENTO WHERE Id_Departamento = '".$filtro."'")->queryAll();
        return $resp;
	...
	public function relations()
	{
		...
		return array(
			...
			'tHINVENTARIOs' => array(self::HAS_MANY, 'THINVENTARIO', 'Id_Departamento'),
		);
	}
	...
	public function search()
	{
	    ...
	    if(empty($this->orderby)){
			$criteria->order = 't.Id_Departamento DESC'; 	
		}else{
			switch ($this->orderby) {
			    case 1:
			        $criteria->order = 't.Id_Departamento ASC'; 
			        break;
			    case 2:
			        $criteria->order = 't.Id_Departamento DESC'; 
			        break;
