...
$query ="
  SET NOCOUNT ON
  EXEC [dbo].[COM_RT_INV_LINEA_FECH]
  @FECHA1 = N'".$FechaM1."',
  @FECHA2 = N'".$FechaM2."',
...
if($opcion == 1){
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Rentabilidad_inv_linea_'.date('Y-m-d H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValue('A1', 'LÍNEA / ITEM');
  ...
  foreach ($query1 as $reg1) {
    ...
    if($linea != $linea_act){
      ...
      if($linea != ""){
        ...
        $objPHPExcel->setActiveSheetIndex()->setCellValue('B'.$Fila, 'TOTAL '.$linea);
  ...
  $n = 'Rentabilidad_inv_linea_'.date('Y-m-d H_i_s');
