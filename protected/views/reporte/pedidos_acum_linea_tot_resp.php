...
$query= "
    SET NOCOUNT ON
    EXEC [dbo].[COM_ACUM_VT_LN_EST_TIP_SUM]
    @OPT = ".$opc.",
    @VAR1 = N'".$linea."',
...
if($opcion == 1){
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Control_pedidos_acumulado_linea_tot_'.date('Y-m-d H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValue('R1', 'CANT. TOTAL');
  $objPHPExcel->setActiveSheetIndex()->setCellValue('S1', 'VLR. TOTAL');
  $objPHPExcel->setActiveSheetIndex()->setCellValue('T1', 'PROM. VENTAS');
  $objPHPExcel->setActiveSheetIndex()->setCellValue('U1', 'STOCK MESES');
  $objPHPExcel->setActiveSheetIndex()->setCellValue('V1', 'BASE PEDIDOS');
  $objPHPExcel->setActiveSheetIndex()->setCellValue('W1', 'EXIST. A LA FECHA');
  $objPHPExcel->setActiveSheetIndex()->setCellValue('X1', 'O.C PEND.');
  $objPHPExcel->setActiveSheetIndex()->setCellValue('Y1', 'A.D PEDIR');
  $objPHPExcel->setActiveSheetIndex()->setCellValue('Z1', '# DÍAS CUBRIM.');
  ...
  foreach ($query1 as $reg1) {
    ...
    $objPHPExcel->setActiveSheetIndex()->setCellValue('R'.$Fila, $CI_CAN);
    $objPHPExcel->setActiveSheetIndex()->setCellValue('S'.$Fila, $CI_VAL);
    ...
    $objPHPExcel->setActiveSheetIndex()->setCellValue('V'.$Fila, $BASE_PEDIDOS);
    $objPHPExcel->setActiveSheetIndex()->setCellValue('W'.$Fila, $EXIST_FECHA);
    $objPHPExcel->setActiveSheetIndex()->setCellValue('X'.$Fila, $O_C_PEND);
    $objPHPExcel->setActiveSheetIndex()->setCellValue('Y'.$Fila, $AD_PEDIR);
    $objPHPExcel->setActiveSheetIndex()->setCellValue('Z'.$Fila, $DIAS_CUB);

    $objPHPExcel->getActiveSheet()->getStyle('A'.$Fila.':D'.$Fila)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
    $objPHPExcel->getActiveSheet()->getStyle('E'.$Fila.':S'.$Fila)->getNumberFormat()->setFormatCode('0'); 
    $objPHPExcel->getActiveSheet()->getStyle('T'.$Fila.':Z'.$Fila)->getNumberFormat()->setFormatCode('#,#0.0');
    $objPHPExcel->getActiveSheet()->getStyle('E'.$Fila.':Z'.$Fila)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
  ...
  $n = 'Control_pedidos_acumulado_linea_tot_'.date('Y-m-d H_i_s');
