...
if($marca_inicial == "||"){
  ...
  $q_mi = Yii::app()->db->createCommand("SELECT Criterio_Descripcion FROM Qlik.dbo.ED_CRITERIOS_ITEMS where Id_Criterio='".$marca_inicial."'")->queryRow();
...
if($marca_final == "||"){
  ...
  $q_mf = Yii::app()->db->createCommand("SELECT Criterio_Descripcion FROM Qlik.dbo.ED_CRITERIOS_ITEMS where Id_Criterio='".$marca_final."'")->queryRow();
...
$query= "
EXEC [dbo].[EC_COM_REDES_FECH_MARCA]
...
if($opcion == 1){
  ...
  class PDF extends FPDF{
    ...
    function Header(){
      ...
      $this->Cell(130,5,utf8_decode('PEDIDOS PENDIENTES POR DESPACHO / MARCA (ECUADOR)'),0,0,'L');
    ...
    function Tabla(){
      ...
      foreach ($query1 as $reg1) {
        ...
        if($marca != $marca_act){
          ...
          $marca = $reg1['MARCA'];
          $q_m = Yii::app()->db->createCommand("SELECT Criterio_Descripcion FROM Qlik.dbo.ED_CRITERIOS_ITEMS where Id_Criterio='".$marca."'")->queryRow();
          $dm = $q_m['Criterio_Descripcion'];
          $texto_marca_final = $dm;
          $this->SetFont('Arial','B',6);
          $this->Ln();
          $this->Cell(195,8, $dm ,0,0,'L');
          $this->Ln();
        }
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Ped_pend_des_marca_ecuador_'.date('Y-m-d H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  //se imprime el total de la ultima marca
  $objPHPExcel->setActiveSheetIndex()->setCellValue('D'.$Fila, 'TOTAL '.$dm);
  $objPHPExcel->setActiveSheetIndex()->setCellValue('E'.$Fila, $Cant_Ped_sp);
  $objPHPExcel->setActiveSheetIndex()->setCellValue('F'.$Fila, $Cant_Env_sp);
  $objPHPExcel->setActiveSheetIndex()->setCellValue('G'.$Fila, $Cant_Redes_sp);
  $objPHPExcel->setActiveSheetIndex()->setCellValue('H'.$Fila, $Cant_Pend_sp);
  ...
  $n = 'Ped_pend_des_marca_ecuador_'.date('Y-m-d H_i_s');
