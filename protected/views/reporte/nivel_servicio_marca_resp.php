...
if($opcion == 1){
  ...
  class PDF extends FPDF{
    ...
    function Header(){
      ...
      $this->Cell(130,5,utf8_decode('PEDIDOS PENDIENTES POR DESPACHO / MARCA'),0,0,'L');
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Ped_pend_des_marca_'.date('Y-m-d H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $n = 'Ped_pend_des_marca_'.date('Y-m-d H_i_s');
