...
if($mi == "||"){
  ...
  $marca_inicial = $mi;
  $marcas = Yii::app()->db->createCommand("SELECT Criterio_Descripcion FROM Qlik.dbo.PR_CRITERIOS_ITEMS WHERE Id_Plan='PL4' AND Id_Criterio='".$marca_inicial."'")->queryRow();
...
if($mf == "||"){
  ...
  $marca_final = $mf;
  $marcas = Yii::app()->db->createCommand("SELECT Criterio_Descripcion FROM Qlik.dbo.PR_CRITERIOS_ITEMS WHERE Id_Plan='PL4' AND Id_Criterio='".$marca_final."'")->queryRow();
  $dm = $marcas['Criterio_Descripcion'];
...
$query ="
  SET NOCOUNT ON
  EXEC [Qlik].[dbo].[PR_COM_RT_INV_MARCA_FECH]
  @FECHA1 = N'".$FechaM1."',
  @FECHA2 = N'".$FechaM2."',
...
if($opcion == 1){
  ...
  class PDF extends FPDF{
    ...
    function Header(){
      ...
      $this->Cell(200,5,utf8_decode('RENTABILIDAD DE INVENTARIO POR MARCA (PERÚ)'),0,0,'L');
    ...
    function Tabla(){
      ...
      foreach ($query1 as $reg1) {
        ...
        if(strlen($reg1['POR_UTIL_BRUTA']  * 100) > 6) {
          $PORC_UTILIDAD   = '0.00';
        }else{
        ...
        if($marca != $marca_act){
          ...
          if($marca != ""){
            ...
            $marcas = Yii::app()->db->createCommand("SELECT Criterio_Descripcion FROM Qlik.dbo.PR_CRITERIOS_ITEMS WHERE Id_Plan='PL4' AND Id_Criterio='".$marca."'")->queryRow();
            $dm = $marcas['Criterio_Descripcion'];
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Rentabilidad_inv_marca_peru_'.date('Y-m-d H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $n = 'Rentabilidad_inv_marca_peru_'.date('Y-m-d H_i_s');
