...
//ora
$query_ora = Yii::app()->db->createCommand("SELECT DISTINCT Id_Criterio, Criterio_Descripcion FROM TH_CRITERIOS_ITEMS WHERE Id_Plan=950 AND Id_Criterio = '".$ora."'")->queryRow();
...
$query ="
  SET NOCOUNT ON
  EXEC [dbo].[COM_RT_L560_ORACLE_FECH]
  @FECHA1 = N'".$FechaM1."',
  @FECHA2 = N'".$FechaM2."',
...
if($opcion == 1){
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Rentabilidad_item_l560_'.date('Y-m-d H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $n = 'Rentabilidad_item_l560_'.date('Y-m-d H_i_s');
