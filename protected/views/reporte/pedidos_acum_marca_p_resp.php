...
foreach ($array_marcas as $a_marcas => $valor) {
  ...
  if($valor == '||'){
    ...
    $marcas = Yii::app()->db->createCommand("SELECT Criterio_Descripcion FROM Qlik.dbo.PR_CRITERIOS_ITEMS where Id_Plan='PL4' AND Id_Criterio='".$valor."'")->queryRow();
...
$query= "
    SET NOCOUNT ON
    EXEC [Qlik].[dbo].[PR_COM_ACUM_VT_MC_EST]
...
if($opcion == 1){
  ...
  class PDF extends FPDF{
    ...
    function Header(){
      ...
      $this->Cell(200,5,utf8_decode('CONTROL DE PEDIDOS ACUMULADO / MARCA (PERÚ)'),0,0,'L');
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Control_pedidos_acumulado_marca_peru_'.date('Y-m-d H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $n = 'Control_pedidos_acumulado_marca_peru_'.date('Y-m-d H_i_s');
