...
$query= "
    SET NOCOUNT ON
    EXEC [dbo].[COM_ACUM_VT_OR_EST_TIP]
    @OPT = ".$opc.",
    @VAR1 = N'".$oracle."',
...
if($opcion == 1){
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Control_pedidos_acumulado_oracle_'.date('Y-m-d H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $n = 'Control_pedidos_acumulado_oracle_'.date('Y-m-d H_i_s');
