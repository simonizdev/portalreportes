...
if($opcion == 1){
  ...
  class PDF extends FPDF{
    ...
    function Header(){
      ...
      $this->Cell(100,5,utf8_decode('Facturación Tiendas Web'),0,0,'L');
      $this->SetFont('Arial','',9);
      $this->Cell(85,5,utf8_decode($this->fecha_actual),0,0,'R');
      $this->Ln();
      $this->SetFont('Arial','',7);
      $this->Cell(185,5,utf8_decode('Criterio de búsqueda: Fecha del '.$this->fecha_inicial.' al '.$this->fecha_final),0,0,'L');
      $this->Ln();
      $this->Ln();
      ...
      $this->Cell(20,2,utf8_decode('PRECIO'),0,0,'L');
      $this->Ln(3);
    ...
    function Tabla(){
      ...
      foreach ($query1 as $reg1) {
        ...
        $this->Cell(20,3,number_format(($PRECIO),0,".",","),0,0,'R');
        $this->Ln(); 
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Fact_tiendas_web_'.date('Y-m-d H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValue('F1', 'CANTIDAD');
  $objPHPExcel->setActiveSheetIndex()->setCellValue('G1', 'PRECIO');
  ...
  foreach ($query1 as $reg1) {
    ...
    $objPHPExcel->setActiveSheetIndex()->setCellValue('F'.$Fila, $CANTIDAD);
    $objPHPExcel->setActiveSheetIndex()->setCellValue('G'.$Fila, $PRECIO);

    $objPHPExcel->getActiveSheet()->getStyle('F'.$Fila.':G'.$Fila)->getNumberFormat()->setFormatCode('0');
  ...
  $n = 'Fact_tiendas_web_'.date('Y-m-d H_i_s');
