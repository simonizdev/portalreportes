...
if($mi == "||"){
  ...
  $marca_inicial = $mi;
  $marcas = Yii::app()->db->createCommand("SELECT Criterio_Descripcion FROM Qlik.dbo.ED_CRITERIOS_ITEMS where Id_Criterio='".$marca_inicial."'")->queryRow();
...
if($mf == "||"){
  ...
  $marca_final = $mf;
  $marcas = Yii::app()->db->createCommand("SELECT Criterio_Descripcion FROM Qlik.dbo.ED_CRITERIOS_ITEMS where Id_Criterio='".$marca_final."'")->queryRow();
  $dm = $marcas['Criterio_Descripcion'];
...
$query ="
  SET NOCOUNT ON
  EXEC [dbo].[EC_COM_RT_INV_MARCA_FECH]
  @FECHA1 = N'".$FechaM1."',
  @FECHA2 = N'".$FechaM2."',
...
if($opcion == 1){
  ...
  class PDF extends FPDF{
    ...
    function Header(){
      ...
      $this->Cell(200,5,'RENTABILIDAD DE INVENTARIO POR MARCA (ECUADOR)',0,0,'L');
      ...
      foreach ($query1 as $reg1) {
        ...
        if($marca != $marca_act){
          ...
          if($marca != ""){
            ...
            $marcas = Yii::app()->db->createCommand("SELECT Criterio_Descripcion FROM Qlik.dbo.ED_CRITERIOS_ITEMS where Id_Criterio='".$marca."'")->queryRow();
            $dm = $marcas['Criterio_Descripcion'];

            $this->SetFont('Arial','B',6);
            $this->Cell(50,5,'TOTAL '.$dm,0,0,'R');
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Rentabilidad_inv_marca_ecuador_'.date('Y-m-d H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  foreach ($query1 as $reg1) {
    ...
    if($marca != $marca_act){
      ...
      if($marca != ""){
        ...
        $marcas = Yii::app()->db->createCommand("SELECT Criterio_Descripcion FROM Qlik.dbo.ED_CRITERIOS_ITEMS where Id_Criterio='".$marca."'")->queryRow();
        $dm = $marcas['Criterio_Descripcion'];
        $objPHPExcel->setActiveSheetIndex()->setCellValue('B'.$Fila, 'TOTAL '.$dm);
        ...
        $objPHPExcel->getActiveSheet()->getStyle('C'.$Fila.':Q'.$Fila)->getNumberFormat()->setFormatCode('#,##0.00');
        $objPHPExcel->getActiveSheet()->getStyle('A'.$Fila.':Q'.$Fila)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
      ...
      $marcas = Yii::app()->db->createCommand("SELECT Criterio_Descripcion FROM Qlik.dbo.ED_CRITERIOS_ITEMS where Id_Criterio='".$marca."'")->queryRow();
      $dm = $marcas['Criterio_Descripcion'];
      $objPHPExcel->setActiveSheetIndex()->setCellValue('A'.$Fila, $dm);
  ...
  $n = 'Rentabilidad_inv_marca_ecuador_'.date('Y-m-d H_i_s');
