...
$query ="
  SET NOCOUNT ON
  EXEC [dbo].[COM_RT_INV_ORACLE_FECH]
  @FECHA1 = N'".$FechaM1."',
  @FECHA2 = N'".$FechaM2."',
...
if($opcion == 1){
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Rentabilidad_inv_oracle_'.date('Y-m-d H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValue('A1', 'ORACLE / ITEM');
  ...
  foreach ($query1 as $reg1) {
    ...
    if($oracle != $oracle_act){
      ...
      if($oracle != ""){
        ...
        $objPHPExcel->setActiveSheetIndex()->setCellValue('B'.$Fila, 'TOTAL '.$oracle);
      ...
      $Fila = $Fila + 1;
      $oracle = $reg1['ORACLE'];
      $objPHPExcel->setActiveSheetIndex()->setCellValue('A'.$Fila, $oracle);
  ...
  $n = 'Rentabilidad_inv_oracle_'.date('Y-m-d H_i_s');
