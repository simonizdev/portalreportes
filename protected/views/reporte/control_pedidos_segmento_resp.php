...
$query= "
    SET NOCOUNT ON
    EXEC [dbo].[COM_CONT_PED_SEGMENTO]
    @OPT = ".$opc.",
    @VAR1 = N'".$segmento."',
...
if($opcion == 1){
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Control_pedidos_segmento_'.date('Y-m-d H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  foreach ($query1 as $reg1) {
    ...
    $objPHPExcel->setActiveSheetIndex()->setCellValue('F'.$Fila, $PROM_VENTAS);
    $objPHPExcel->setActiveSheetIndex()->setCellValue('G'.$Fila, $STOCK_MESES);
    $objPHPExcel->setActiveSheetIndex()->setCellValue('H'.$Fila, $BASE_PEDIDOS);
    $objPHPExcel->setActiveSheetIndex()->setCellValue('I'.$Fila, $EXIST_FECHA);
    $objPHPExcel->setActiveSheetIndex()->setCellValue('J'.$Fila, $O_C_PEND);
    $objPHPExcel->setActiveSheetIndex()->setCellValue('K'.$Fila, $AD_PEDIR);
    $objPHPExcel->setActiveSheetIndex()->setCellValue('L'.$Fila, $DIAS_CUB);
  ...
  $n = 'Control_pedidos_segmento_'.date('Y-m-d H_i_s');
