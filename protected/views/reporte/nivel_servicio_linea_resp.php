...
if($opcion == 1){
  ...
  class PDF extends FPDF{
    ...
    function Header(){
      ...
      $this->Cell(130,5,utf8_decode('PEDIDOS PENDIENTES POR DESPACHO / LÍNEA'),0,0,'L');
    ...
    function Tabla(){
      ...
      foreach ($query1 as $reg1) {
        ...
        if($linea != $linea_act){
          ...
          if($linea != ""){
            $this->SetFont('Arial','B',5);
            $this->Cell(88,5,'TOTAL '.$linea,0,0,'R');
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Ped_pend_des_linea_'.date('Y-m-d H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $n = 'Ped_pend_des_linea_'.date('Y-m-d H_i_s');
