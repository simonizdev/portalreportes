...
$query_ev = Yii::app()->db->createCommand("SELECT Criterio_Descripcion FROM TH_CRITERIOS_CLIENTES WHERE Id_Plan=300 AND Id_Criterio = ".$ev)->queryRow();
...
$query ="
SET NOCOUNT ON
EXEC [dbo].[COM_RT_CLIENTE_560_FECH_EST]
  @FECHA1 = N'".$FechaM1."',
  @FECHA2 = N'".$FechaM2."',
  @ESTRUCTURA  = N'".$ev."'
";
...
if($opcion == 1){
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Rentabilidad_estructura_560_'.date('Y-m-d H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValue('A1', 'CLIENTE');
  $objPHPExcel->setActiveSheetIndex()->setCellValue('B1', 'ORACLE');
  ...
  foreach ($query1 as $reg1) {
    ...
    if($MARCA != "NOTAS CREDITO"){   
      ...
      $objPHPExcel->setActiveSheetIndex()->setCellValue('A'.$Fila, $CLIENTE);
      $objPHPExcel->setActiveSheetIndex()->setCellValue('B'.$Fila, $MARCA);
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValue('B'.$Fila, 'NOTAS CRÉDITO');
  $objPHPExcel->setActiveSheetIndex()->setCellValue('C'.$Fila, $NOTAS_st);

  $objPHPExcel->getActiveSheet()->getStyle('C'.$Fila)->getNumberFormat()->setFormatCode('#,##0.00');
  $objPHPExcel->getActiveSheet()->getStyle('A'.$Fila.':C'.$Fila)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
  ...
  $n = 'Rentabilidad_estructura_560_'.date('Y-m-d H_i_s');
