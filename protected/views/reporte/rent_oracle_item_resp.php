...
$marca_inicial = trim($model->des_ora_ini);
$marca_final = trim($model->des_ora_fin);
...
$query ="
EXEC [dbo].[COM_RENT_FECH_ORACLE]
  @FECHA1 = N'".$FechaM1."',
  @FECHA2 = N'".$FechaM2."',
  @ORACLE1 = N'".$marca_inicial."',
  @ORACLE2 = N'".$marca_final."'
";
...
if($opcion == 1){
  ...
  class PDF extends FPDF{
    ...
    function Header(){
      ...
      $this->Cell(200,5,'Rentabilidad por oracle / item',0,0,'L');
      $this->SetFont('Arial','',9);
      $this->Cell(140,5,utf8_decode($this->nombre_empresa.', '.$this->fecha_actual),0,0,'R');
      $this->Ln();
      $this->SetFont('Arial','',9);
      $this->Cell(340,5,utf8_decode('Criterio de búsqueda: Fecha del '.$this->fecha_inicial.' al '.$this->fecha_final.' / Desc. oracle de '.$this->marca_inicial.' a '.$this->marca_final),0,0,'L');
      $this->Ln();
      $this->Ln();
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Rentabilidad_oracle_item_'.date('Y-m-d H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $n = 'Rentabilidad_oracle_item_'.date('Y-m-d H_i_s');
